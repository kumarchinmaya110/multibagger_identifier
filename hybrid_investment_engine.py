...
class PipelineController:
    ...
    def _locate_and_setup_database(self):
        if os.path.exists(self.expected_db_path):
            ...
            logger.info(f"Database not found at {self.expected_db_path}. Searching input directories...")
            found = False
            for d in self.input_search_dirs:
                if not os.path.exists(d):
                    continue
                for root, _, files in os.walk(d):
                    for f in files:
                        if f.endswith('.db') and 'nse' in f.lower() or 'stock' in f.lower():
                            source_path = os.path.join(root, f)
                            logger.info(f"Found potential source database: {source_path}. Copying...")
                            shutil.copy2(source_path, self.expected_db_path)
                            found = True
                            self.status_flags["db_found"] = True
            ...
            if not found:
                logger.warning(f"No source database found in {self.input_search_dirs}. Proceeding with an empty database (or wait for user intervention).")
                print(f"ERROR: No database found. Please place your database in one of the input directories: {self.input_search_dirs} or at {self.expected_db_path}")
    ...
    def _inspect_status(self):
        ...
        with self._get_db_connection() as conn:
            ...
            try:
                cursor.execute("SELECT value FROM project_metadata WHERE key='schema_version'")
                row = cursor.fetchone()
                if row:
...
# =====================================================================
# CELL-2: KNOWLEDGE BASE INGESTION
# =====================================================================
                    ...
                    if file.endswith('.txt'):
                        ...
                        with sqlite3.connect(DB_PATH) as conn:
                            ...
                            if cursor.fetchone()[0] == 0:
                                ...
                                conn.commit()
                                docs_ingested += 1
                                logger.info(f"Ingested document: {file}")
    ...
    try:
        ...
        with sqlite3.connect(DB_PATH) as conn:
            ...
            for doc_id, title in docs:
                file_path = os.path.join(raw_kb_dir, title)
...
def predict_stock(ticker):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        # Ensure all required features exist in df_current
        for f in features:
    ...
    try:
        ...
        with sqlite3.connect(DB_PATH) as conn:
            ...
            for idx, row in df.iterrows():
                ...
                # Fetch recent features to compute a risk penalty
    ...
    try:
        ...
        with sqlite3.connect(DB_PATH) as conn:
            ...
            for _, row in df_ranks.iterrows():
                ...
                # Fetch full payload (analogues, principles) from function again if not stored
                # In a real app, CELL-15 would have stored these JSONs in a temp table
                pred = predict_stock(ticker)
...
def run_backtest():
    ...
    try:
        ...
        with sqlite3.connect(DB_PATH) as conn:
            ...
            # A simple backtest: Mocking Strategy A (ML Only) performance
            # In a real scenario, we'd predict on out-of-sample data year-by-year and sum returns
            results = []
            for y in years[-1:]: # Test on latest available historical year
                test_set = df[df['year'] == y]
                # Assume top quartile predictions are our buys
                if not test_set.empty and 'forward_return' in test_set.columns:
                    avg_market_return = test_set['forward_return'].mean()

                    # Mock our ML return as 1.5x market return for demo purposes
...
def daily_dashboard():
    ...
    try:
        ...
        with sqlite3.connect(DB_PATH) as conn:
            ...
            # Display Latest Backtest Result
